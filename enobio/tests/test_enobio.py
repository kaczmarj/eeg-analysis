...
base_dir = op.join('data')
raw_fname = op.join(base_dir, 'test_raw.easy')
...
def test_io_easy():
    """Test importing NeuroElectrics .easy files"""
    from pandas import read_csv
    with warnings.catch_warnings(record=True) as w1:
        warnings.simplefilter('always')
        # main tests, and test missing event_id
        _test_raw_reader(read_raw_enobio, input_fname=raw_fname,
                         montage=montage)
        # _test_raw_reader(read_raw_enobio, input_fname=raw_fname_onefile,
        #                  montage=montage)
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        raw0 = read_raw_enobio(input_fname=raw_fname, montage=montage,
                               event_id=event_id, preload=True)
        # raw1 = read_raw_enobio(input_fname=raw_fname, montage=montage,
        #                        event_id=event_id, preload=False)
        raw2 = read_raw_enobio(input_fname=raw_fname_onefile, montage=montage,
                               event_id=event_id)
        raw3 = read_raw_enobio(input_fname=raw_fname, montage=montage,
                               event_id=event_id)
